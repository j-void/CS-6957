...
if __name__ == "__main__":
    ...
    model = VisionEncoderDecoderModel.from_encoder_decoder_pretrained("google/vit-base-patch16-224-in21k", "prajjwal1/bert-mini")    
    ...
    for epoch in tqdm(range(cfg.num_epochs), desc="Epochs: "):
        ...
        for idx, (pixel_values, labels, attention_mask) in enumerate((tqdm(train_loader, desc="Batch: "))):
            ...
            final_loss = model(pixel_values=pixel_values, labels=labels, decoder_attention_mask=attention_mask).loss
            final_loss.backward()
            optimizer.step()
