...
if __name__ == "__main__":
    ...
    train_dataset = ParsingDataset(data_file=cfg.train_data_path, pose_set=pose_set, tagset=tagset, glove=glove, split='train')
    ...
    for epoch in tqdm(range(cfg.num_epochs), desc="Epochs: "):
        ...
        for idx, (word_embeds, pos_labels, action_label) in enumerate((tqdm(train_loader, desc="Batch: "))):
            optimizer.zero_grad()
        ...
        current_val_acc, _ = util.get_accuracy(model=model, data_loader=val_loader, reverse_tagset=reverse_tagset, device=cfg.DEVICE)
    ...
    las, uas = util.get_accuracy(model=best_val_model, data_loader=test_loader, reverse_tagset=reverse_tagset, device=cfg.DEVICE)
    tqdm.write("Test Accuracies : las="+str(las)+", uas="+str(uas))
    util.run_hidden_data(model=best_val_model, data_loader=hidden_loader, \
        reverse_tagset=reverse_tagset, path=os.path.join(save_imdt, "hidden_output.txt"), device=cfg.DEVICE)
